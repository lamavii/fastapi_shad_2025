...
@pytest.mark.asyncio
async def test_get_books(db_session, async_client, add_seller):
...
@pytest.mark.asyncio
async def test_get_single_book(db_session, async_client, add_seller):
    # Создаем книги вручную, а не через ручку, чтобы нам не попасться на ошибку которая
    # может случиться в POST ручке
    seller = add_seller
    book = books.Book(author="Pushkin", title="Eugeny Onegin", year=2001, count_pages=104, seller_id=seller.id)
...
@pytest.mark.asyncio
async def test_delete_book(db_session, async_client, add_seller):
    # Создаем книги вручную, а не через ручку, чтобы нам не попасться на ошибку которая
    # может случиться в POST ручке
    seller = add_seller
    book = books.Book(author="Pushkin", title="Eugeny Onegin", year=2001, count_pages=104, seller_id=seller.id)
...
@pytest.mark.asyncio
async def test_update_book(db_session, async_client, add_seller):
    # Создаем книги вручную, а не через ручку, чтобы нам не попасться на ошибку которая
    # может случиться в POST ручке
    seller = add_seller
